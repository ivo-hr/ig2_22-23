...
bool IG2App::keyPressed(const OgreBites::KeyboardEvent& evt)
{
  ...
  else if (evt.keysym.sym == SDLK_p) 
  {
	  ...
	  if (mAvionNode != nullptr) {
		  auto box = mAvionNode->_getWorldAABB();
		  for (auto d : avispas) {
			  if (d != nullptr) {
				  auto dBox = d->_getWorldAABB();
				  if (box.intersects(dBox)) {
					  d->setVisible(false);
					  d->_setDerivedPosition(Vector3(0, 0, 0));
					  vivas--;

					  if (vivas <= 0) avion->AvionWin();
				  }
			  }
		  }
	  }
...
/// <param name=""></param>
  ...
  sinbad = new Sinbad(mSinbadNode, 2);
  mSinbadNode->scale(5, 5, 5);
  mSinbadNode->setPosition(0, 125, 0);
